...
func TestPromptReloadEndToEnd(t *testing.T) {
	...
	systemPrompt := buildSystemPrompt(10000.0, 10, 5, "test_strategy", true)
	...
	newSystemPrompt := buildSystemPrompt(10000.0, 10, 5, "test_strategy", true)
...
func TestPromptReloadWithCustomPrompt(t *testing.T) {
	...
	result := buildSystemPromptWithCustom(10000.0, 10, 5, customPrompt, false, "base", true)
	...
	// 测试2: 覆盖基础 prompt
	result = buildSystemPromptWithCustom(10000.0, 10, 5, customPrompt, true, "base", true)
	if strings.Contains(result, baseContent) {
...
func TestPromptReloadFallback(t *testing.T) {
	...
	result := buildSystemPrompt(10000.0, 10, 5, "nonexistent", true)
	if !strings.Contains(result, defaultContent) {
		t.Errorf("请求不存在的模板时，未降级到 default")
	}

	// 测试2: 空模板名，应该使用 default
	result = buildSystemPrompt(10000.0, 10, 5, "", true)
	if !strings.Contains(result, defaultContent) {
