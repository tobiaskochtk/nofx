...
func buildAutonomousOptimizerProposalConversationUserReplay(payload map[string]any) string {
	...
	if patterns := parseAutonomousOptimizerNestedObject(payload, "learned_patterns"); len(patterns) > 0 {
		replay["learned_patterns"] = map[string]any{
			...
			"monitoring_rule_count":               patterns["monitoring_rule_count"],
			"lifecycle_degrading_count":           patterns["lifecycle_degrading_count"],
			"lifecycle_rollback_watch_count":      patterns["lifecycle_rollback_watch_count"],
			"lifecycle_expired_count":             patterns["lifecycle_expired_count"],
			"lifecycle_fragile_count":             patterns["lifecycle_fragile_count"],
			"lifecycle_lagging_guard_count":       patterns["lifecycle_lagging_guard_count"],
			"live_guard_protective_count":         patterns["live_guard_protective_count"],
			"live_guard_overblocking_count":       patterns["live_guard_overblocking_count"],
			"live_guard_improving_count":          patterns["live_guard_improving_count"],
			"live_guard_degrading_count":          patterns["live_guard_degrading_count"],
			"live_guard_newly_overblocking_count": patterns["live_guard_newly_overblocking_count"],
			...
			"top_fragile_monitoring_rules":        limitAutonomousOptimizerReplayArray(patterns["top_fragile_monitoring_rules"], 3),
			"top_overblocking_monitoring_rules":   limitAutonomousOptimizerReplayArray(patterns["top_overblocking_monitoring_rules"], 3),
			"top_improving_monitoring_rules":      limitAutonomousOptimizerReplayArray(patterns["top_improving_monitoring_rules"], 3),
			"top_degrading_monitoring_rules":      limitAutonomousOptimizerReplayArray(patterns["top_degrading_monitoring_rules"], 3),
		}
	}
