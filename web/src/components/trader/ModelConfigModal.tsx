...
import type { AIModel, RemoteModelInfo } from '../../types'
...
export function ModelConfigModal({
  ...
  const usesAmbientAuth =
    selectedModel?.provider === 'codex' || selectedModel?.id === 'codex'
  const isApiKeyRequired = !isN8n && !usesAmbientAuth
  ...
  useEffect(() => {
    ...
    if (editingModelId && modelDetails) {
      ...
      setModelName(
        modelDetails.customModelName ||
          (modelDetails.provider === 'codex' || modelDetails.id === 'codex'
            ? AI_PROVIDER_CONFIG.codex.defaultModel
            : '')
      )
    }
  ...
  useEffect(() => {
    ...
    void api
      ...
      .then((items) => {
        if (cancelled) return
        setAvailableRemoteModels(items)
        setModelName((current) => {
          const trimmed = current.trim()
          if (trimmed) return trimmed
          return items[0]?.id || AI_PROVIDER_CONFIG.codex.defaultModel
        })
      })
      .catch(() => {
        if (cancelled) return
        setAvailableRemoteModels([])
        setModelName(
          (current) => current.trim() || AI_PROVIDER_CONFIG.codex.defaultModel
        )
      })
  ...
  }, [selectedModelId, selectedModel?.provider, selectedModel?.id])
  ...
  const handleSelectModel = (modelId: string) => {
    ...
    setApiKey('')
    setBaseUrl('')
    if (nextModel?.provider === 'codex' || nextModel?.id === 'codex') {
      setModelName(AI_PROVIDER_CONFIG.codex.defaultModel)
    } else {
      setModelName('')
    }
    setCurrentStep(1)
          ...
          {(currentStep === 1 || editingModelId) &&
            ...
            selectedModel.id !== 'claw402' && (
              <StandardProviderConfigForm
                ...
                availableRemoteModels={availableRemoteModels}
                apiKey={apiKey}
                baseUrl={baseUrl}
                modelName={modelName}
                editingModelId={editingModelId}
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        setKeyError(
          pickText({
            zh: '验证请求失败',
            de: 'Validierungsanfrage fehlgeschlagen',
            en: 'Validation request failed',
          })
        )
      } finally {
        setValidating(false)
        ...
        {hasExistingWallet && (
          ...
          >
            ...
            >
              {pickText({
                zh: '已自动提取当前钱包',
                de: 'Aktuelles Wallet automatisch geladen',
                en: 'Current wallet loaded automatically',
              })}
            </div>
            <div className="text-[11px] leading-5" style={{ color: '#A0AEC0' }}>
              {pickText({
                zh: '你现在可以直接查看当前钱包地址、余额和充值二维码。只有在想更换钱包时，才需要重新输入新的私钥。',
                de: 'Du kannst jetzt direkt Wallet-Adresse, Guthaben und Einzahlungs-QR-Code sehen. Gib nur dann einen neuen privaten Schlüssel ein, wenn du dieses Wallet ersetzen willst.',
                en: 'You can view the current wallet address, balance, and deposit QR code right away. Only enter a new private key if you want to replace this wallet.',
              })}
            </div>
            {!configuredModel?.walletAddress && localWalletAddress ? (
              <div className="mt-2 text-[10px]" style={{ color: '#848E9C' }}>
                {pickText({
                  zh: '当前地址来自本地已保存的新手钱包。',
                  de: 'Diese Adresse stammt aus dem lokal gespeicherten Anfänger-Wallet.',
                  en: 'This address comes from the locally saved beginner wallet.',
                })}
              </div>
            ) : null}
            {!configuredModel?.walletAddress &&
            ...
            serverWalletAddress ? (
              <div className="mt-2 text-[10px]" style={{ color: '#848E9C' }}>
                {pickText({
                  zh: '当前地址来自后端保存的钱包配置。',
                  de: 'Diese Adresse stammt aus der auf dem Server gespeicherten Wallet-Konfiguration.',
                  en: 'This address comes from the wallet saved on the server.',
                })}
              </div>
            ) : null}
          </div>
        ...
        <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder={
                hasExistingWallet
                  ? pickText({
                      zh: '如需切换钱包，请手动输入新的私钥',
                      de: 'Gib nur dann einen neuen privaten Schlüssel ein, wenn du das Wallet wechseln willst',
                      en: 'Enter a new private key only if you want to switch wallets',
                    })
                  : '0x...'
              }
          ...
          {hasExistingWallet && !apiKey ? (
            <div className="text-[11px] leading-5" style={{ color: '#848E9C' }}>
              {pickText({
                zh: '后续这里只使用你第一次创建并保存的钱包；如果你要换钱包，请手动填写新的私钥。',
                de: 'Hier wird weiterhin das Wallet verwendet, das du beim ersten Mal erstellt und gespeichert hast. Gib nur dann manuell einen neuen privaten Schlüssel ein, wenn du das Wallet wechseln willst.',
                en: 'This screen keeps using the wallet created and saved the first time. Enter a new private key manually only if you want to switch wallets.',
              })}
            </div>
          ) : null}
              ...
              <>
                ...
                >
                  ...
                  >
                    ...
                    {pickText({
                      ...
                      de: 'Bitte bestätige, dass dies deine Wallet-Adresse ist (z. B. in MetaMask prüfen)',
                ...
                {resolvedUsdcBalance !== null && (
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    >
                      {showDeposit
                        ? pickText({ zh: '收起', de: 'Ausblenden', en: 'Hide' })
                        : pickText({
                            zh: '💳 充值',
                            de: '💳 Einzahlen',
                            en: '💳 Deposit',
                ...
                {showDeposit && (
                  ...
                  >
                    ...
                    >
                      ...
                      {pickText({
                        zh: '充值 USDC (Base 链)',
                        de: 'USDC einzahlen (Base-Netzwerk)',
                    ...
                    <div className="flex gap-3 items-start mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        >
                          {pickText({
                            zh: '扫码或复制地址转账',
                            de: 'QR-Code scannen oder Adresse kopieren, um zu überweisen',
                    ...
                    >
                      <div>
                        📱{' '}
                        {pickText({
                          zh: '用交易所 App 扫描二维码直接转账',
                          de: 'Scanne den QR-Code mit deiner Börsen-App für die Überweisung',
                      ...
                      <div>
                        ...
                        {pickText({
                          zh: '提币时网络选择 Base',
                          de: 'Wähle beim Auszahlen das Base-Netzwerk',
                      ...
                      <div>
                        ...
                        {pickText({
                          zh: '最低充值 $1 USDC 即可开始',
                          de: 'Mindestens 1 USDC zum Starten',
                          en: 'Min $1 USDC to start',
                        })}
                      </div>
                    </div>
                  </div>
                ...
                {!apiKey && hasExistingWallet && (
                  <div className="text-[11px]" style={{ color: '#848E9C' }}>
                    {pickText({
                      zh: '当前正在使用这个钱包充值。若要切换钱包，再输入新的私钥并保存即可。',
                      de: 'Dieses Wallet wird aktuell zum Aufladen verwendet. Gib nur dann einen neuen privaten Schlüssel ein, wenn du das Wallet wechseln willst.',
                      en: 'This wallet is currently used for funding. Enter a new private key only if you want to switch wallets.',
                    })}
                  </div>
                )}
                {claw402Status && (
...
function StandardProviderConfigForm({
  ...
  availableRemoteModels: RemoteModelInfo[]
  apiKey: string
  baseUrl: string
  modelName: string
  editingModelId: string | null
  ...
  const isCodex =
    selectedModel.provider === 'codex' || selectedModel.id === 'codex'
  const isApiKeyRequired = !isN8n && !isCodex
  const resolvedCodexModel =
    modelName.trim() ||
    AI_PROVIDER_CONFIG[selectedModel.provider]?.defaultModel ||
    'gpt-5.4'
  const remoteModelOptions = [...availableRemoteModels]
  if (!remoteModelOptions.some((item) => item.id === resolvedCodexModel)) {
    remoteModelOptions.unshift({
      id: resolvedCodexModel,
      label: resolvedCodexModel,
    ...
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      {!isCodex && (
        <div className="space-y-2">
          ...
          >
            ...
            {isApiKeyRequired ? 'API Key *' : 'API Key'}
          </label>
          <input
            type="password"
            value={apiKey}
      ...
      <div className="space-y-2">
        ...
        {isCodex ? (
          ...
          >
            <NofxSelect
              value={resolvedCodexModel}
              onChange={onModelNameChange}
              options={remoteModelOptions.map((item) => ({
                value: item.id,
