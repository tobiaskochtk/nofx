...
export const configApi = {
  ...
  async getAvailableRemoteModels(modelId: string): Promise<RemoteModelInfo[]> {
    const result = await httpClient.get<{ items: RemoteModelInfo[] }>(
      `${API_BASE}/models/${modelId}/available-models`
    )
    if (!result.success) throw new Error('Failed to fetch remote model catalog')
